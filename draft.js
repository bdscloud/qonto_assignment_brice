...
/*
async function getGiphy4(table){
  console.log(api_url + '&q=bob');
  for (var i = 0 ; i < table.length -1 ; i++){
    request(api_url + '&q=bob', { json: true }, (err, res, body) => {
...
async function getGiphy2(table){
  var listToReturn = [] ; 
  let promiseList = [] ;
  for (var i = 0 ; i < table.length -1 ; i++){
    var promisei = new Promise((resolve,reject) => {
      request({
        urli : api_url + '&q=' + table.rows[i].firstname.toString().trim() + "%20" + table.rows[i].category.toString().trim() + "%20" + table.rows[i].trimed_city.toString().trim() , 
        method: 'GET'
      },
      (err, res, body) => {if (err) reject(err)} ,
        promiseList.push(body.data)
